...
| **Design 2** | **Product PWA** — customer, owner, worker (when built) | **TBD** — add `refs/design-2/DESIGN.md` when the app look is locked |

**Do not mix Design 1 and Design 2.** Salon pages and `/owner` are never agency landings. The marketing site is never pixel-bento marketing chrome on product screens.
...
- Building **PWA / salon / owner** → `docs/mvp/04-UI-Design-Goals.md` and `.cursor/rules/frontend/` until `refs/design-2/DESIGN.md` exists.
...
- **Design 2:** not started — create `refs/design-2/` when the product design is ready.
