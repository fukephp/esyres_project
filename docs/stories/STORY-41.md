...
- `/create-salon` is a registered guest-reachable route (not `/signup`, not under `/owner`). Sparse Design-1 form: AuthShell if logged out, email-verify gate if needed, then salon name. No homepage hero/footer, no feature grid, no owner-panel chrome. Brand in a shared top-nav (empty slot) is STORY-43.
...
- Shared top-nav chrome (STORY-43)
