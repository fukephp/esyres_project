...
- `/salon/:id` — salon profile (picker + optional assistant). Address when set (omit if missing). Services grouped under owner service category headings (`md+` jump list when ≥2 visible groups, inside the left services block). One idle `Pošalji zahtjev` (`Odaberi usluge, dan i vrijeme.` when idle): under the title on a phone; `md+` compact sticky right booking sidebar (pill spans that column; chat alternate in that block). Picker and chat submit share that black pill on the left `max-w-md` form. Open weekday hours rows seed the picker’s preferred date (left seven-row list, no hours rail). Never the homepage. Top-nav matches `/salons` (Moje rezervacije only).
