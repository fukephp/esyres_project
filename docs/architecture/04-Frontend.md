...
- `/salon/:id` — salon profile (picker + optional assistant). Address when set (omit if missing). Idle `Pošalji zahtjev` under the title (`Odaberi usluge, dan i vrijeme.` under that button only) and again above chat. All four `Pošalji zahtjev` share one black pill. Open weekday hours rows seed the picker’s preferred date (one column, no rail). Never the homepage. Top-nav matches `/salons` (Moje rezervacije only).
