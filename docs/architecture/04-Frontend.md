...
- `/owner` is a lazy pending queue + Worker Availability Panel for one salon-day (login only; Prihvati on named-worker rows via `acceptPreferredTime`; drag onto a free cell calls `proposeTime`; Odbi on every pending row via two-step `declineBooking`; Predloži opens `/owner/requests/:id`). Salon context is `?salon=` when the owner has more than one salon; omit or a bad id → first owned (`id` ASC). `/owner/requests/:id` is Request Detail (lazy owner route): form `proposeTime`, Prihvati, Odbi. No `/owner` link on customer pages.
