...
- `/salons` — discovery home (nearby / Popular of listed salons). Idle **discovery teaser** (first few as cards); a discovery chip, name search, or show all opens **discovery results** (hairline rows: name, today’s busy-level, that salon’s service category names, address when set). Top-nav brand links to `/`; slot is Moje rezervacije. Geolocation mounts here. See `docs/adr/0030-discovery-teaser-then-results.md`.
...
- `/salon/:id` — salon profile (picker + optional assistant). Address when set (omit if missing). Services grouped under owner service category headings (`md+` jump list when ≥2 visible groups, inside the left services block; browse-only on the page). No idle `Pošalji zahtjev` on load. Open weekday rows seed the preferred date; the pill appears under Radno vrijeme and opens a picker modal (phone sheet / `md+` card). `md+` sticky right aside is empty. Chat alternate is a Pitaj salon card under hours (messenger bubbles + bottom composer; scripted). Picker and chat submit share the black pill. Never the homepage. Top-nav matches `/salons` (Moje rezervacije only).
- `/bookings?verified=1` — landing after a successful email-verify signed GET (banner on My Bookings). `?verify=invalid` (bad or expired signature) and `?verify=mismatch` (session is a different user). No dedicated `/verify-email` route. Top-nav slot: empty when logged out; name + Odjava (`button-destructive`) when logged in.
- `/owner` — owner: inbox, worker panel (home; occupying cells show the current job), in-flight chat tab (`/owner/chats`, list + optional Take over / Release + DND toggle), Basic Stats (`/owner/stats`: last-7-day bookings + all-time QR scan/visit/conversion; `?salon=` like chats; nav `Statistika`); **salon catalog** (`/owner/salons`, open now; OwnerNav **Saloni** on every owner route); **add salon** (`/owner/salons/create`); **salon edit** (`/owner/salons/:id`, exclusive Informacije / Radno vrijeme / Usluge / Radnici chips; Radno vrijeme one-column exclusive accordion, land all collapsed); **salon switcher** when the user owns more than one salon (`?salon=` on queue, chats, and stats only). Shared top-nav overlays above aside + OwnerNav (name + Odjava as `button-destructive`).

Owner chunks (including `@dnd-kit`) are lazy-loaded so the customer first paint does not ship the grid.

Customer browse has no login wall. The homepage is not a login wall. Login/register appears in the homepage top-nav slot, at request submit, My Bookings, `/create-salon`, and owner routes. Register asks for person name first (Ime i prezime); login does not. AuthShell shows **Rezervacije** on customer shells and **Panel** on `/create-salon` and `/owner*` (including those email-verify screens). Homepage auth hides hero + footer while open. Logged-in Odjava is Cal `button-destructive` (same size as Panel). Logged-in `/` also shows Moje rezervacije (name → Moje rezervacije → Odjava → Panel). Owner routes overlay the shared top-nav (name + Odjava) above OwnerNav.
