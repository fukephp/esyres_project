...
- `/salon/:id` — salon profile (picker + optional assistant). Address when set (omit if missing). Idle `Pošalji zahtjev` under the title and again above chat. Open weekday hours rows seed the picker’s preferred date (one column, no rail). Never the homepage. Top-nav matches `/salons` (Moje rezervacije only).
