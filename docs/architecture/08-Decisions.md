...
48. **Day-first salon chrome** — open weekday tap seeds the preferred date; `Pošalji zahtjev` appears under Radno vrijeme and opens a picker modal (not idle on load, not in the `md+` aside). Logged-in `/` adds Moje rezervacije between name and Odjava. See `docs/adr/0034-day-first-salon-booking-chrome.md`.
