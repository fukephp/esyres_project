...
41. **Homepage is in the PWA** — typed `/` is the homepage (header + hero + footer); discovery home is a named route. No `esyres_app/marketing/`, no `/welcome`, no long-scroll marketing page. `/salon/:id` and `GET /qr/{id}` skip homepage chrome. See `docs/adr/0025-homepage-named-discovery.md` (supersedes the same-URL overlay in `docs/adr/0024-company-pitch-in-pwa.md`).
