...
25. **QR guest cookie ~7 days** — last salon wins; physical scan is `GET /qr/{salonId}` (not `/salon/:id`); reconcile when a session user has both verification **timestamps** and still holds the cookie (QR GET, `verifyPhoneOtp`, sessioned email GET, `login`). Local `hasVerified*` skip does not count. See `docs/adr/0017-qr-sticker-is-not-salon-profile.md` and `docs/adr/0018-qr-reconnect-requires-timestamps.md`.
