...
36. **App root is `esyres_app/`** — Laravel at that folder, product PWA at `esyres_app/frontend/`, marketing at `esyres_app/marketing/`. No git-root `backend/` or `frontend/`. Slim Compose in `esyres_app/` is `php` + `node` + **mysql** (Behat dedicated test DB). Decision 26’s remaining services (nginx, redis, worker, reverb, vite, mailpit) stay the target. See `docs/adr/0001-mysql-in-slim-compose.md`.
