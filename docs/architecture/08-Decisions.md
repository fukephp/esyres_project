...
36. **App root is `esyres_app/`** — Laravel at that folder, product PWA at `esyres_app/frontend/`, marketing at `esyres_app/marketing/`. No git-root `backend/` or `frontend/`. Slim Compose in `esyres_app/` is `php` + **vite** + **mysql** (Behat dedicated test DB). Verify is `docker compose up -d` then `exec -T`, not `compose run`. Decision 26’s remaining services (nginx, redis, worker, reverb, mailpit) stay the target. See `docs/adr/0001-mysql-in-slim-compose.md` and `docs/adr/0012-long-lived-slim-compose.md`.
