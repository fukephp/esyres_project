...
7. **Public salon register** — a signed-in user may create a salon (and another from the panel); invite-only is revoked. See `docs/adr/0026-public-salon-register.md`. Discovery lists only **listed** salons (`docs/adr/0027-listed-salons-on-discovery.md`). Local demo seed (STORY-38) still provisions known logins.
...
17. **Behat + Vitest + Playwright** — Behat-only backend gate (no Pest, no parallel PHPUnit suite). Behat is GraphQL-over-HTTP, not Mink; verify with `vendor/bin/behat --format=progress --stop-on-failure` (CLI flags; not `behat.yml`). Playwright remains frontend E2E. No Storybook.
...
36. **App root is `esyres_app/`** — Laravel at that folder, product PWA at `esyres_app/frontend/`. No sibling marketing site. No git-root `backend/` or `frontend/`. Slim Compose in `esyres_app/` is `php` + **vite** + **mysql** + **reverb** (Behat dedicated test DB). Verify is `docker compose up -d` then `exec -T`, not `compose run`. Decision 26’s remaining services (nginx, redis, worker, mailpit) stay the target. See `docs/adr/0001-mysql-in-slim-compose.md`, `docs/adr/0012-long-lived-slim-compose.md`, `docs/adr/0014-reverb-in-slim-compose.md`, and `docs/adr/0024-company-pitch-in-pwa.md`.
...
41. **Homepage is in the PWA** — typed `/` is the homepage (header + hero + footer); discovery home is `/salons`. No `esyres_app/marketing/`, no `/welcome`, no long-scroll marketing page. `/salon/:id` and `GET /qr/{id}` skip homepage chrome. See `docs/adr/0025-homepage-named-discovery.md` (supersedes the same-URL overlay in `docs/adr/0024-company-pitch-in-pwa.md`).
