...
41. **Homepage is in the PWA** — `/` is the Bosnian homepage (header + existing hero + simple footer), not a one-shot pitch gate. Pronađi salon goes to `/salons`. No `esyres_app/marketing/`, no `/welcome`, no owner waitlist. `/salon/:id` and `GET /qr/{id}` skip the homepage. See `docs/adr/0027-homepage-not-pitch-gate.md` (supersedes same-URL pitch routing in `docs/adr/0024-company-pitch-in-pwa.md`).
