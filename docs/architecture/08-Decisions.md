...
44. **Shared top-nav** — one full-bleed Cal bar (brand → `/`, per-route native slot) on guest and owner routes. Salon/discovery slot is Moje rezervacije, not homepage CTAs. Owner overlay does not replace OwnerNav. See `docs/adr/0029-shared-top-nav.md`.
45. **Discovery teaser then results** — idle `/salons` is a short card teaser from Nearby or Popular in Sarajevo; show all, a category chip, or name search opens hairline results (name, today’s busy, categories, address when set). Not a featured ranking and not photo cards. See `docs/adr/0030-discovery-teaser-then-results.md`.
46. **Owner salon catalog** — `/owner/salons` (open now from hours). Add salon is `/owner/salons/create` (name and address; does not change `/create-salon`). Salon edit is `/owner/salons/:id`. Current job is occupying-cell labels on the panel, not the catalog. See `docs/adr/0031-add-salon-requires-address.md` and `docs/adr/0032-owner-salon-catalog.md`.
