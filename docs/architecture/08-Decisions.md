...
25. **QR guest cookie ~7 days** — last salon wins; physical scan is `GET /qr/{salonId}` (not `/salon/:id`); reconcile when a session user is email+phone verified and still holds the cookie (QR GET, `verifyPhoneOtp`, sessioned email GET, `login`). See `docs/adr/0017-qr-sticker-is-not-salon-profile.md`.
