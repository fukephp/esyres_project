...
41. **Company pitch is in the PWA** — typed `/` first paint is one Bosnian Design 1 screen, then discovery home on the same URL. No `esyres_app/marketing/`, no `/welcome`, no owner waitlist. `/salon/:id` and `GET /qr/{id}` skip the pitch. See `docs/adr/0024-company-pitch-in-pwa.md`.
