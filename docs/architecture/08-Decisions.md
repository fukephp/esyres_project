...
39. **Owner marks no-show after start** — stay `confirmed` + `no_show_at`; still occupying; overlay cleared. Not auto, not a sixth status, not from cancel. See `docs/adr/0019-owner-marks-no-show-after-start.md`.
40. **Trust counters increment on the event** — `cancel_count` / `late_cancel_count` / `no_show_count` on that customer and that salon in the same transaction as cancel or mark-no-show. Booking row is the idempotency stamp. See `docs/adr/0020-trust-counters-increment-on-event.md`.
