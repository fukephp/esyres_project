...
- **User** — email+password; person name (`name`, Ime i prezime) required at register (not derived from the email local-part); `email_verified_at`; optional `phone` (E.164, any country, unique when present) + `phone_verified_at`; roles: can be customer and owner. Integer trust counters `cancel_count` / `late_cancel_count` / `no_show_count` (default 0; incremented in the event transaction, see `docs/adr/0022-trust-counters-increment-on-event.md`). See `docs/adr/0006-phone-e164-any-country.md`.
- **Salon** — `owner_id`, profile, address, optional salon description, `lat`/`lng`, hours, breaks, holidays, `cancellation_notice_hours`, `reschedule_cap` (default 1), `dnd` (default false; turns take-over off), optional main image and gallery extras on disk. Same three trust counters as User (per salon). One user may own many salons (separate profiles, not a chain-location product). `createSalon` is name-only on `/create-salon`; **add salon** sets name and address. Listed on discovery only with one open weekday and one service. Nearby still needs `lat`/`lng`. Owner `updateSalon` writes name, address (required, cannot blank), and optional description. Main image and gallery upload/remove immediately via GraphQL multipart (owner edit query returns URLs; public `salon` query does not this slice). See `docs/adr/0037-salon-media-on-informacije.md`.
- **Worker** — belongs to a salon; assigned to services (matrix later; MVP owner UI is name on salon edit). Inherits salon hours. Not a user. No delete in the salon-edit slice.
- **Service category** — belongs to salon; owner-typed name; a service belongs to exactly one. Optional migrate-only legacy key (`HAIR` / `MAKE_UP` / `MASSAGE`) so discovery chips still match old rows; owner never sets it. See `docs/adr/0033-salon-service-categories.md`.
- **Service** — belongs to salon and to one service category; `duration_minutes` (default 30), price feninga. No HAIR / MAKE_UP / MASSAGE enum on the service.
- **Booking** — `salon_id`, `customer_id`, optional `worker_id` (null = no preference until accept/propose), `preferred_date`, `preferred_starts_at`, status, `proposed_starts_at`, `proposed_worker_id`, duration derived from services (sum, rounded up to 15 minutes), decline/expire reason. Optional overlay `reschedule_date` / `reschedule_starts_at` on a **confirmed** row (in-progress reschedule; does not occupy). Customer cancel of confirmed sets `cancelled` and snapshots `cancelled_at` plus late (inside `cancellation_notice_hours` at cancel time). `owner_responded_at` is set once on the first successful accept, counter-propose, or decline (see `docs/adr/0007-owner-responded-at-on-first-action.md`). Optional `no_show_at` (nullable; owner mark after start; status stays `confirmed`; still occupies; see `docs/adr/0021-owner-marks-no-show-after-start.md`). Optional `reminder_day_sent_at` / `reminder_hour_sent_at` (nullable; set when that reminder mail is sent; cleared on accept-reschedule). Optional `CreateBookingInput.intakeToken` may attach an in-flight `AssistantIntake` (does not change the status machine). Converted intake is the origin signal (no `bookings.origin`); owner GraphQL may nest it on `Booking` when the session owns the salon.
