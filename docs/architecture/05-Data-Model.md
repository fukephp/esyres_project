...
- **Salon** — `owner_id`, profile, address, `lat`/`lng`, hours, breaks, holidays, `cancellation_notice_hours`, reschedule cap, `dnd` (default false; turns take-over off), photos on disk. One user may own many salons (separate profiles, not a chain-location product).
...
- **AssistantIntake** — salon-scoped scripted-chat snapshot that is not a request yet (`token` UUID for the guest, bigint PK internally). In-flight while `booking_id` is null and `updated_at` is within 24h. Optional `taken_over_at` pauses guest upsert/chat-send while the salon is open and DND is off (`takenOver` is that pause **in effect**, not the raw flag). Converted when chat `createBooking` attaches `booking_id`. Not a message log; Request Detail transcript is the labeled snapshot (services, worker, day, time).
