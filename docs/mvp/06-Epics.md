...
Salon profile setup (self-serve create salon on the same account — `/create-salon`, name only), owner salon catalog (`/owner/salons`, open now from hours), add salon (`/owner/salons/create`, name and address), salon edit (`/owner/salons/:id`: exclusive Informacije / Radno vrijeme / Usluge / Radnici chips; name, address, full weekly hours including cancel window — Radno vrijeme is a one-column exclusive accordion, land all collapsed; create/edit service categories and services and workers by name; delete a category only when empty; no delete of services or workers, no assignment matrix), salon switcher if the owner has more than one salon.
