...
On the salon profile, the **primary** button stays the picker (`Pošalji zahtjev`): one idle send (plus `Odaberi usluge, dan i vrijeme.` when idle) — under the title on a phone; `md+` compact sticky right booking sidebar. Picker submit and chat submit share that same black pill on the left form. Scripted chat is a visible alternate (`Nisi sigurna? Pitaj salon.`) in that idle block, not a second product. Open weekday hours rows (left column) seed the picker’s day. Owners may market the assistant; guests who already know still get a few taps.
...
- **Guest inner column.** `/`, `/salons`, `/salon/:id`, `/create-salon`, `/bookings` share one ~1200px column (Design 1): top-nav inner and `main` align (same padding, `mx-auto`). Hairline bar stays full-bleed. Lists and headings span; copy/forms/CTAs keep left-aligned local max-widths (not a second centered `max-w-md` page). Discovery stays 1-col. `/salon/:id` on a phone stays one column (idle send under the title). `md+` may split hours/services/form left and a compact sticky **booking sidebar** right — not an hours rail, not a second scroll column on a phone, not the owner aside. Open weekday rows stay in the left column and seed the picker. Exception: service list may show a `md+` jump list of service category names (not hours) when the salon has ≥2 visible groups; that list stays in the left services block. Owner overlay inner stays unconstrained. Sparse still means no homepage hero/footer on other routes.
- **Salon `Pošalji zahtjev` chrome.** One idle send, picker submit, and chat submit are one black pill (Inter 600, 48px min-height, press `#242424` + slight scale). Idle pill spans the compact sidebar (`md+`) or the under-title measure (phone). Idle only gets the support line `Odaberi usluge, dan i vrijeme.` `md+` booking sidebar may `sticky` inside `main`. CSS press only — no GSAP, no sticky TopNav, no phone dock, no second primary color.
