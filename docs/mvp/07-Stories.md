...
- As a customer, I want to pick a preferred day only (not a time), so that I don't have to guess at the salon's real availability.
...
- As a customer, I want confirmation that my request was sent and is awaiting a proposed time, so that I know what to expect next.
...
- As an owner, I want to drag a pending request onto an open slot on a worker's row, so that I can propose an exact time in one motion.
...
- As a customer, I want to approve, reject, or ask for a different day once a time is proposed, so that I stay in control of the final appointment. Asking for a different day updates the same request (new day, back to pending), not a duplicate.
