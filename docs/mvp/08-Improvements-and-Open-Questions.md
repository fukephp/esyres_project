...
Not a WhatsApp auto-book product and not an LLM in v1. Esyres stays a two-sided marketplace. The in-PWA scripted assistant is 24/7 intake into the existing `requested` inbox so Zahtjevi is worth opening daily. Scheduling authority stays with the owner.
...
- Owner home stays Zahtjevi (month navigator + selected-day list). Chat is a tab + badge for in-flight. After send, the object is the booking; transcript on Request Detail.
...
Typed `/` is the Bosnian homepage (top-nav homepage slot + existing hero + Pronađi salon + simple footer). Pronađi salon goes to `/salons`. Shared Cal top-nav on `/`, `/salons`, `/salon/:id`, `/create-salon`, `/bookings`, and as an overlay on `/owner` (ADR 0029). Logged-in with person name: `Dobrodošli, {name}` in every of those slots (never email). Guest routes share a ~1200px inner column (nav inner + main align); owner bar inner unconstrained. No persist-seen. `/salon/:id` and `GET /qr/{id}` never show the homepage page. `esyres_app/marketing/` stays gone. No owner waitlist, Formspree, `/invite`, public pricing page, or Awwwards/GSAP. One Design 1 pack (owner keeps dense Zahtjevi chrome, Cal light). Self-serve create salon is `/create-salon`, not a waitlist. An owner adds another shop from `/owner/salons/create` (name and address; does not change the public create).
