...
- **Homepage** — typed `/` is a permanent Bosnian page: shared top-nav with the homepage slot (logo, Prijava/Registracija, Get your panel → `/create-salon` or Panel → `/owner` as the black primary), the existing hero + Pronađi salon, simple footer. Not a login wall. Pronađi salon goes to `/salons`. `/salon/:id` and the QR sticker skip the homepage page. No persist-seen, no separate marketing site, no owner waitlist.
...
- **Registration / login** — email + password. Guest browse has no login wall. Homepage top-nav slot offers Prijava/Registracija (same `AuthShell`); request submit and My Bookings still have it. Email verification is required before a request can be sent (and before owner routes or create salon). Phone is optional at register (encouraged); **phone OTP is required to send a request and to respond to a counter-proposal**. Verified phone also enables OTP as an alternate login. `phone_verified_at` is captured now; reward-badge **display** is Phase 2.
