...
- **Worker Availability Panel** — table of workers × **15-minute** slots per day. Occupying cells show the **current job** (service snapshot names on that worker’s row). Pending requests sit in a queue at the top of each day; the owner **accepts** a preferred time in one tap or **drags** a request onto an open slot to counter-propose (sets `time_proposed`, not `confirmed`). A counter-proposal **holds that slot** until confirm, decline, or expire. `requested` does not occupy a clock slot.
...
- **Staff/Worker Management** — add/edit workers by name on salon edit; every worker has a row on the panel. Workers inherit salon hours (no per-worker shift editor or vacation calendar at MVP). Worker↔service assignment is later, not this slice.
