...
- **Top-nav** — one full-bleed Cal bar (brand → `/`, per-route native slot) on `/`, `/salons`, `/salon/:id`, `/create-salon`, `/bookings`, and as an overlay on `/owner`. Discovery and salon share Moje rezervacije in the slot (not Prijava/Get your panel). Create-salon slot is empty. My Bookings: name + Odjava when logged in. Owner overlay: name + Odjava; OwnerNav stays.
