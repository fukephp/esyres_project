...
- **Homepage** — typed `/` is a permanent Bosnian page: shared top-nav with the homepage slot (logo, Prijava/Registracija, Get your panel → `/create-salon` or Panel → `/owner` as the black primary), the existing hero + Pronađi salon, simple footer. Not a login wall. When Prijava/Registracija is open, hide hero + footer; the auth `h1` is Rezervacije; form stays left. Pronađi salon goes to `/salons`. `/salon/:id` and the QR sticker skip the homepage page. No persist-seen, no separate marketing site, no owner waitlist.
- **Top-nav** — one full-bleed Cal bar (brand → `/`, per-route native slot) on `/`, `/salons`, `/salon/:id`, `/create-salon`, `/bookings`, and as an overlay on `/owner`. Guest routes share one inner column (~1200px, Design 1) so nav inner and `main` align; owner overlay inner stays unconstrained. Discovery and salon share Moje rezervacije in the slot (not Prijava/Get your panel). Create-salon slot is empty. Logged-in Odjava is Cal `button-destructive` (`error-strong` fill, white label, same size as Panel; press darker, no hover, one tap) on `/`, `/bookings`, and `/owner*`. Homepage order: name → Moje rezervacije → Odjava → Panel. Prijava/Registracija stay text. Owner overlay: name + that Odjava; OwnerNav stays (Saloni → `/owner/salons`).
- **Discovery home** — `/salons`; no login required; geolocation-based "salons near you," falling back to "Popular in Sarajevo". Idle is a short **discovery teaser** (first few listed salons as cards). A **discovery chip**, name search (`Ime salona`), or show all opens **discovery results** (hairline rows). Teaser and results show name, today’s busy-level, that salon’s service category names, and address when set. Only **listed** salons (one open weekday and one service). Brand in the top-nav links home. See `docs/adr/0030-discovery-teaser-then-results.md`.
- **Salon Profile** — address when set (omit if missing; text line, no maps in this slice), working hours (open weekday rows seed the picker’s date; seven-row list in the left column; helper `Odaberi dan da pošalješ zahtjev.`; no hours rail), service list grouped under the owner’s service category names (heading per group with ≥1 service; `md+` jump list when ≥2 groups; browse-only on the page), and a busy-level badge (🟢/🟡/🔴 — see below). No idle `Pošalji zahtjev` on load. After an open weekday is selected, the pill appears under Radno vrijeme and opens a picker **modal** (services, worker, date, time, send). `md+` sticky right aside stays empty. Chat alternate is a Pitaj salon card under hours (messenger thread: salon bubbles left, guest right, bottom composer chrome; still scripted). Picker and chat submit use the same black pill. Photos and optional maps link-out stay later; no in-app map SDK.
- **Busy-Level Indicator** — coarse signal of how booked a day is, shown on the salon profile and on discovery teaser/results instead of a detailed time grid. Deliberately hides per-slot scheduling detail from customers.
...
- **Registration / login** — email + password. Register requires person name (Ime i prezime, GraphQL `name`) as the first field; login has no name field. Guest browse has no login wall. Homepage top-nav slot offers Prijava/Registracija (same `AuthShell`); request submit and My Bookings still have it. AuthShell (and panel email-verify) shows a place heading: **Rezervacije** on customer shells, **Panel** on `/create-salon` and `/owner*` (logged-out owner must not use queue/chat/stats titles for that). Email verification is required before a request can be sent (and before owner routes or create salon). Phone is optional at register (encouraged); **phone OTP is required to send a request and to respond to a counter-proposal**. Verified phone also enables OTP as an alternate login. `phone_verified_at` is captured now; reward-badge **display** is Phase 2.
...
- **Worker Availability Panel** — one full-width Cal **day card** on `/owner` for the selected salon-day: date header, that day’s pending queue as chips, then a **15-minute** job board with **hours down** and **workers as columns**. Occupying cells show the **current job** (service snapshot names on that worker’s column). The owner **accepts** a preferred time in one tap or **drags** a request onto an open slot to counter-propose (sets `time_proposed`, not `confirmed`). A counter-proposal **holds that slot** until confirm, decline, or expire. `requested` does not occupy a clock slot. Not a guest month-and-pills widget.
...
- **Salon catalog** — `/owner/salons`. List of shops the owner owns; each row shows **open now** from working hours (not listed, not a toggle). OwnerNav **Saloni** on every owner route. Not the salon switcher and not discovery.
- **Add salon** — `/owner/salons/create` (already an owner). Name and address both required. Lands on salon edit. Does not change `/create-salon` (first shop, name only).
- **Salon edit** — `/owner/salons/:id`. Four exclusive chips (Informacije / Radno vrijeme / Usluge / Radnici); one panel at a time; land and refresh on Informacije. Full-width hairline box in owner `main`. Name and address (cannot blank), full weekly hours (days, times, optional break, cancel window; Radno vrijeme is a one-column exclusive accordion, land all collapsed), create/edit service categories and services (Usluge: selected group + its services; rename; delete category only if empty) and workers by name. No delete of services or workers. No worker↔service matrix. DND stays on chats. Photos, coordinates, and reschedule cap stay off this page.
- **Working Hours & Availability Settings** — edited on salon edit (weekly template + cancel window). Radno vrijeme: one column of weekday summaries; at most one day editor open. Holidays and reschedule-cap UI stay off this page. Default reschedule cap remains 1 in-progress overlay per booking.
...
- **Staff/Worker Management** — add/edit workers by name on salon edit; every worker has a column on the panel. Workers inherit salon hours (no per-worker shift editor or vacation calendar at MVP). Worker↔service assignment is later, not this slice.
