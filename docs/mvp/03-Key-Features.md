...
- **Salon Profile** — address when set (omit if missing; text line, no maps in this slice), working hours (open weekday rows seed the picker’s date; seven-row list in the left column, no hours rail), service list grouped under the owner’s service category names (heading per group with ≥1 service; `md+` jump list when ≥2 groups), and a busy-level badge (🟢/🟡/🔴 — see below). One idle `Pošalji zahtjev` (with `Odaberi usluge, dan i vrijeme.` when idle): under the title on a phone; `md+` compact sticky right booking sidebar (pill spans that column; chat alternate stays in that block). Picker and chat submit use that same black pill on the left form. Photos and optional maps link-out stay later; no in-app map SDK.
