...
- **Top-nav** — one full-bleed Cal bar (brand → `/`, per-route native slot) on `/`, `/salons`, `/salon/:id`, `/create-salon`, `/bookings`, and as an overlay on `/owner`. Guest routes share one inner column (~1200px, Design 1) so nav inner and `main` align; owner overlay inner stays unconstrained. Discovery and salon share Moje rezervacije in the slot (not Prijava/Get your panel). Create-salon slot is empty. My Bookings: name + Odjava when logged in. Owner overlay: name + Odjava; OwnerNav stays (Saloni → `/owner/salons`).
