...
- **Salon Profile** — address when set (omit if missing; text line, no maps in this slice), working hours (open weekday rows seed the picker’s date; still one column, no rail), service list grouped under the owner’s service category names (heading per group with ≥1 service; `md+` jump list when ≥2 groups), and a busy-level badge (🟢/🟡/🔴 — see below). Idle `Pošalji zahtjev` under the title (with `Odaberi usluge, dan i vrijeme.`) and again above chat; picker and chat submit use that same black pill. Photos and optional maps link-out stay later; no in-app map SDK.
