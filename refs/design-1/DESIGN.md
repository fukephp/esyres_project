...
- Discovery and salon keep this sparse layout. Do not restyle them as homepage hero/footer. They share the ~1200px guest column; sparse is not a 448px page. Forms and CTAs may keep a left-aligned local max-width. They share the top-nav with Moje rezervacije in the slot (not Prijava/Get your panel). Salon hours stay a seven-row list in the left column (tappable open days, muted closed days); no schedule rail. `md+` salon may add a compact sticky booking sidebar (idle send + hint + chat alternate); phone stays one column with that send under the title. Salon `Pošalji zahtjev` (one idle, picker submit, chat submit) is the product pill: Inter 600, white on `#111111`, min-height 48px, press `#242424` and slight scale; idle may have one muted support line. Not a second primary color, not GSAP, not a sticky TopNav or phone dock.
