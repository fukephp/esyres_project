...
test('path helpers for salon, create-salon, bookings, owner', () => {
  ...
  expect(isOwnerPath('/owner/salons/create')).toBe(true)
  expect(isOwnerPath('/owner/salons/1')).toBe(true)
...
test('topNavSlot by path', () => {
  ...
  expect(topNavSlot('/owner/salons/create')).toBe('session')
  expect(topNavSlot('/owner/salons/1')).toBe('session')
