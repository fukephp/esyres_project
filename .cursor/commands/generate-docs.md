...
3. Update existing files in place. Do not invent new `STORY-xx` files (only a product grill or `/new-story` creates those). Do not add other new doc files unless the user asked, or the existing sets are the wrong place. Do not restore `docs/tasks/`.
