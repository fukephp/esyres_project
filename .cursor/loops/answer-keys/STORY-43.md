...
- [x] `topNavSlot(path)` is `home` only for `/` and `''`; `discovery` for `/salons` and `/salon/:id`; `empty` for `/create-salon`; `session` for `/bookings` and `/owner` plus `/owner/chats`, `/owner/stats`, `/owner/requests/:id`. `/welcome` and unknown paths are `empty` — verify: Vitest (`esyres_app/frontend/src/lib/homepage.ts`)
- [x] `topNavChrome(path, me)`: brand is always `{ to: '/', brandKey: 'pitch.brand' }`. `home` + no me → login + register + `ownerPanelCta(false)`. `home` + me → `homepageDisplayName` + logout + `ownerPanelCta(me.salons.length > 0)`. `discovery` → bookings link only (no login/register/panel/name/logout). `empty` → brand only. `session` + me → name + logout (no panel, no bookings link). `session` + no me → brand only — verify: Vitest
- [x] i18n `bs`: `nav.bookings` is `Moje rezervacije`; `home.getPanel` stays `Imaš salon? Otvori panel`; `home.panel` stays `Panel`; bar Odjava is `home.logout` `Odjava` (not `bookings.logout`). `pitch.*` hero strings and `auth.login` / `auth.register` unchanged — verify: Vitest
...
- [x] Homepage hero (`pitch.h1` / support / steps / Pronađi salon) and footer (`home.footerCity` / `home.footerLine`) exist only on `Homepage.tsx`. Discovery, salon, create-salon, bookings, and owner files do not import/render that footer or the Pronađi salon CTA — verify: Vitest reading those page files
...
- [x] One React PWA in `esyres_app/frontend/`. No sibling marketing site. No new GraphQL, REST, or booking mutations. `GET /qr/{id}` stays Laravel 302 to `/salon/:id` — verify: no new schema/feature files this PR; `test ! -d esyres_app/marketing`; `QrController.php` / `SpaUrl.php` unchanged; existing Behat stays green
- [x] i18next `bs` only. No Playwright, RTL, Pest, GraphQL codegen, or `vite-plugin-pwa` work this PR. Owner chunks stay lazy in `App.tsx` — verify: `esyres_app/frontend/package.json`; `App.tsx` still `lazy()` for owner pages; no `pestphp` require
- [x] Do not replace `OwnerNav` or restyle discovery/salon as homepage hero/footer. Do not add sticky chrome. Do not put homepage slot CTAs on `/salon/:id` — verify: product checks above + `OwnerNav.tsx` still the aside links
...
**This PR (2026-09-12):** Docker missing. Host PHP 8.3 + host MySQL (`esyres_test` only, `DB_HOST=mysql` → 127.0.0.1). Passed: `php artisan --version`; `vendor/bin/behat --format=progress --stop-on-failure` (442 scenarios); frontend `typecheck` / `test` (120) / `build`; file-absence gates.
