...
- Consult: `.cursor/CONTEXT.md`, `docs/mvp/` (03, 06, 07), `docs/architecture/` (02, 03, 05, 06, 08 #6 #18 #19 #26 #36), `docs/adr/0017-reminder-scan-not-delayed-jobs.md`, `docs/stories/STORY-33.md` plus STORY-11 / 14 / 29 / 30 / 31 / 32, `docs/glossary.md` (**Confirmed booking**, **Reminder**, **Accept reschedule**, **Cancel**, **Ping** ≠ this notify)
...
- **Fire (2026-09-10):** scheduled artisan `bookings:send-reminders` scans confirmed rows. Register `everyMinute` in Laravel’s scheduler. Behat calls the command with frozen time. No delayed-on-confirm jobs (`docs/adr/0017-reminder-scan-not-delayed-jobs.md`). Notification is queued (`ShouldQueue`) like `VerifyEmail`; Behat sync + fake records the send.
