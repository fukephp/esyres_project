...
- Customer: typed `/` is the homepage (shared top-nav with the homepage slot, existing hero, simple footer). Pronađi salon goes to `/salons`. Idle `/salons` is a short card teaser from nearby or Popular in Sarajevo; a discovery chip, name search, or show-all opens the hairline results list (name, today’s busy, that salon’s service category names, address). Shared top-nav also on `/salons`, `/salon/:id`, `/create-salon`, `/bookings` (per-route slot; salon/discovery = Moje rezervacije, not Prijava/Get your panel). Logged-in with person name: `Dobrodošli, {name}` (text, not a link; never email) on `/`, `/salons`, `/salon/:id`, `/create-salon`, `/bookings`, `/owner*`. No name → no identity text. Logged-in `/` and `/bookings`: that greeting + Odjava (Cal `button-destructive`: `error-strong` fill, white label, Panel size; press darker; no hover/confirm). Homepage order greeting → Moje rezervacije → Odjava → Panel. Discovery/salon: greeting then Moje rezervacije (no Odjava/Panel). Create-salon: greeting only when named. Guest routes share one ~1200px inner column so nav and main align; owner overlay inner stays unconstrained. Day-level busy badge (🟢/🟡/🔴), simple date+time picker (no slot grid). `/salon/:id` shows address when set (omit if missing; no maps). Services grouped under owner category headings (`md+` jump list when ≥2 visible groups; browse-only on the page). No idle `Pošalji zahtjev` on load: pick an open weekday (hint `Odaberi dan da pošalješ zahtjev.`), then the pill sits under that selected hours row (later weekdays push down) and opens a picker modal (phone sheet / `md+` card; muted weekday+date under the salon name). `md+` sticky right aside stays empty. Not an hours rail, not the owner aside, not a sticky TopNav or phone dock. Picker and chat submit keep the same black pill. Open weekday hours rows seed the picker’s next date (left seven-row list, not a rail; closed rows stay muted). Chat is the messy-intent alternate on the salon profile (Pitaj salon card under hours opens a picker-like overlay: empty Cal greeting, salon unbubbled left / guest light-gray right pills, dummy bottom composer, scripted chips above it; salon-branded Bosnian, live salon data only, coarse 1–3 time suggestions, same email+phone OTP to send). Native date and time in the chat or picker overlay must not dismiss it or leave Pitaj salon untappable. Choosing a calendar date keeps the overlay and that date; last busy stays until the new day’s busy arrives (no loading page). Email+password login (not a homepage wall). Register requires person name (Ime i prezime); login stays email+password. AuthShell shows Rezervacije (customer) or Panel (create-salon / owner). Verified email + phone OTP required to **send a request**, **respond to a counter-proposal**, **ask to reschedule**, and **cancel** a confirmed booking (skipped when `APP_ENV=local`; staging and production keep the gates).
- Owner: self-serve create salon on `/create-salon` (same account, name only). Owner routes overlay the same top-nav (`Dobrodošli, {person name}` + Odjava when named, same destructive button) above aside + OwnerNav (Saloni → `/owner/salons` on every owner route; **Postavke** → `/owner/settings`, password only, no `?salon=`). Home (Zahtjevi) is one full-width Cal card: month navigator (occupying-only worker-colored dots) plus a selected-day list (pending till-pile, then soon occupying, then the rest including Predloženo vrijeme, titled Pauza / Zatvoreno); one-tap accept; counter-propose from Request Detail only (no drag, no 15-minute worker board); Request Detail (`/owner/requests/:id`) is that same Cal card spanning `main` (one column; Nazad above; in-card name + mode clock; form / read / bounce); occupying rows show the current job (service snapshot names). Salon catalog (`/owner/salons`) is stacked hairline boxes of shops they own (name + open now from hours; **Uredi** → salon edit; name is not a link; plus under the title is the only add). Not listed, not a toggle, not a table. Add salon is `/owner/salons/create` (already an owner; name and address required) then lands on salon edit `/owner/salons/:id`. Salon edit (`/owner/salons/:id`) uses exclusive chips Informacije / Radno vrijeme / Usluge / Radnici (one full-width boxed panel; land on Informacije); name, address, optional salon description, optional main image, and optional gallery of extra images on Informacije (upload/remove immediately; guest profile and public GraphQL stay photoless), full weekly hours (Radno vrijeme: one-column exclusive accordion, land all collapsed), and create/edit service categories and services (Usluge: selected group + its services; rename; delete category only if empty) and workers by name (no delete). DND stays on chats. Coords and reschedule cap stay off salon edit. Discovery stays not photo cards. In-flight chat tab + badge; optional Take over (pause that intake, not live owner messages; guest waits only after that tap; after hours / DND assistant always finishes). Salon switcher if they own more than one salon (`?salon=` on queue/chats/stats only). Complexity stays on this side. A confirmed booking may carry an in-progress reschedule overlay; the original slot stays occupied until the owner accepts.
...
**Local verify** uses the frontend-only classifier, then one of the two command sets.

**Frontend-only classifier (fail-closed):** From the git root, union changed paths: untracked (`git ls-files --others --exclude-standard`), unstaged (`git diff --name-only`), staged (`git diff --cached --name-only`), and this branch vs `main` (`git diff --name-only main...HEAD`; if `main` is missing, `origin/main`). Skip Behat only if **every** path under `esyres_app/` in that union is under `esyres_app/frontend/`. Git-root `docs/`, `DESIGN.md`, `refs/`, `.cursor/`, and `AGENTS.md` do not trigger Behat. Any other `esyres_app/` path (PHP, `features/`, `graphql/`, Composer, Compose, app README, …) → full Behat. Do not skip from a story label. Nearby UI stories are not a class.

**When skipped:** do not `compose up`, start php/mysql, or run `php artisan --version`. From `esyres_app/frontend/`: `npm run typecheck`, `npm run test`, `npm run build` (host npm). `docker compose exec -T vite npm run …` only if that container is already up. No Playwright as a Behat substitute. Layout stays merge-visual (playbook **UI ready rule**).

**When Behat runs** (classifier fails, or the human asks for Behat / `/run-tests --suite`): from `esyres_app/`: `docker compose up -d`; `docker compose exec -T php php artisan --version`; `docker compose exec -T php vendor/bin/behat --format=progress --stop-on-failure` (Behat loads `.env.behat`, DB `esyres_test` only — never `esyres`); frontend `typecheck` / `test` / `build`. Never `docker compose run` for verify or servers; reuse :5173/:8000/:8080. See `esyres_app/README.md`. Backend gate is Behat, not `php artisan test`. Behat flags are CLI-only (same flags on `--suite owner|guest`); do not put them in `behat.yml`. A green run still means the full selected suite passed.

**Cloud Agent verify:** Frontend-only: host npm from `esyres_app/frontend/` only — no host PHP/MySQL. If Behat runs and `docker` is missing or dockerd is nested (apt-install `docker.io` inside the VM), do **not** start Compose MySQL that way — Behat becomes ~14× slower. Same as STORY-36: host PHP + host MySQL, still `.env.behat` / `esyres_test` only. Do not `migrate:fresh` or seed `esyres`. Do not `compose down -v`.
...
| `skills/grill-with-docs/` | User-invoked grilling against the codebase; glossary + ADRs as they lock; product/stories end-batch (`/grill-with-docs`; same opt-in tokens) |
| `skills/new-story/` | Incremental product story; grilling then always persist `STORY-xx` (`/new-story`; no diverge; existing epic only; same opt-in tokens) |
