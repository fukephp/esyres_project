...
| `DESIGN.md` | Index for **one** pack — Design 1 Cal (homepage, discovery, salon, owner). Read before UI. |
...
5. Before UI work, read root `DESIGN.md`, then `refs/design-1/DESIGN.md`. Discovery / salon / `/owner` still follow `docs/mvp/04-UI-Design-Goals.md` and `rules/frontend/` for product UX (dense owner panel, sparse customer). Homepage IA stays on `/` only. Does not override `docs/mvp/` or `rules/frontend/`.
