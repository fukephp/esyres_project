...
- Customer: typed `/` is the homepage (shared top-nav with the homepage slot, existing hero, simple footer). Pronađi salon goes to `/salons`. Idle `/salons` is a short card teaser from nearby or Popular in Sarajevo; a discovery chip, name search, or show-all opens the hairline results list (name, today’s busy, that salon’s service category names, address). Shared top-nav also on `/salons`, `/salon/:id`, `/create-salon`, `/bookings` (per-route slot; salon/discovery = Moje rezervacije, not Prijava/Get your panel). Logged-in `/` and `/bookings`: name + Odjava (Cal `button-destructive`: `error-strong` fill, white label, Panel size; press darker; no hover/confirm). Homepage order name → Odjava → Panel. Guest routes share one ~1200px inner column so nav and main align; owner overlay inner stays unconstrained. Day-level busy badge (🟢/🟡/🔴), simple date+time picker (no slot grid). `/salon/:id` shows address when set (omit if missing; no maps). Services grouped under owner category headings (`md+` jump list when ≥2 visible groups). One idle `Pošalji zahtjev` (never two): under the title on a phone; `md+` compact sticky right booking sidebar (pill spans that column; hint `Odaberi usluge, dan i vrijeme.` when idle; chat alternate stays in that block). Not an hours rail, not the owner aside, not a sticky TopNav or phone dock. Picker and chat submit keep the same black pill on the left `max-w-md` form. Open weekday hours rows seed the picker’s next date (left seven-row list, not a rail; closed rows stay muted). Chat is the messy-intent alternate on the salon profile (salon-branded Bosnian, live salon data only, coarse 1–3 time suggestions, same email+phone OTP to send). Email+password login (not a homepage wall). Register requires person name (Ime i prezime); login stays email+password. AuthShell shows Rezervacije (customer) or Panel (create-salon / owner). Verified email + phone OTP required to **send a request**, **respond to a counter-proposal**, **ask to reschedule**, and **cancel** a confirmed booking (skipped when `APP_ENV=local`; staging and production keep the gates).
