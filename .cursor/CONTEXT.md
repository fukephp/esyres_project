...
- Customer: typed `/` is the homepage (shared top-nav with the homepage slot, existing hero, simple footer). Pronađi salon goes to `/salons`. Idle `/salons` is a short card teaser from nearby or Popular in Sarajevo; a category chip, name search, or show-all opens the hairline results list (name, today’s busy, categories, address). Shared top-nav also on `/salons`, `/salon/:id`, `/create-salon`, `/bookings` (per-route slot; salon/discovery = Moje rezervacije, not Prijava/Get your panel). Logged-in `/` and `/bookings`: name + Odjava (Cal `button-destructive`: `error-strong` fill, white label, Panel size; press darker; no hover/confirm). Homepage order name → Odjava → Panel. Guest routes share one ~1200px inner column so nav and main align; owner overlay inner stays unconstrained. Day-level busy badge (🟢/🟡/🔴), simple date+time picker (no slot grid). `/salon/:id` shows address when set (omit if missing; no maps). Idle `Pošalji zahtjev` also under the title (`Odaberi usluge, dan i vrijeme.` under that header button only). All four `Pošalji zahtjev` (idle header, idle lower, picker submit, chat submit) share one black pill (press + slight scale; CSS only). Open weekday hours rows seed the picker’s next date (one column, not a rail; closed rows stay muted). Chat is the messy-intent alternate on the salon profile (salon-branded Bosnian, live salon data only, coarse 1–3 time suggestions, same email+phone OTP to send). Email+password login (not a homepage wall). Register requires person name (Ime i prezime); login stays email+password. AuthShell shows Rezervacije (customer) or Panel (create-salon / owner). Verified email + phone OTP required to **send a request**, **respond to a counter-proposal**, **ask to reschedule**, and **cancel** a confirmed booking (skipped when `APP_ENV=local`; staging and production keep the gates).
- Owner: self-serve create salon on `/create-salon` (same account, name only). Owner routes overlay the same top-nav (name + Odjava, same destructive button) above aside + OwnerNav (Saloni → `/owner/salons` on every owner route). Pending queue (shows preferred date/time), Worker Availability Panel (one-tap accept + drag-to-counter-propose + tap fallback) is home; occupying cells show the current job (service snapshot names). Salon catalog (`/owner/salons`) lists shops they own with open now from hours (not listed, not a toggle). Add salon is `/owner/salons/create` (already an owner; name and address required) then lands on salon edit `/owner/salons/:id`. Salon edit (`/owner/salons/:id`) uses exclusive chips Informacije / Radno vrijeme / Usluge / Radnici (one full-width boxed panel; land on Informacije); name, address, full weekly hours, and create/edit services and workers by name (rename/update only; no delete). DND stays on chats. Photos, coords, and reschedule cap stay off salon edit. In-flight chat tab + badge; optional Take over (pause that intake, not live owner messages; guest waits only after that tap; after hours / DND assistant always finishes). Salon switcher if they own more than one salon (`?salon=` on queue/chats/stats only). Complexity stays on this side. A confirmed booking may carry an in-progress reschedule overlay; the original slot stays occupied until the owner accepts.
