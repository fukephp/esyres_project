...
- Customer: guest discovery, day-level busy badge (🟢/🟡/🔴), simple date+time picker (no slot grid). Chat is the messy-intent alternate on the salon profile (salon-branded Bosnian, live salon data only, coarse 1–3 time suggestions, same email+phone OTP to send). Email+password login (not a homepage wall). Verified email + phone OTP required to **send a request** and to **respond to a counter-proposal**.
