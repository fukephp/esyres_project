...
Use when the user asks for the homepage, Esyres landing, or marketing homepage on typed `/`. Do not treat discovery, salon, or `/owner` as landings unless asked. Do not scaffold a marketing site or add a `marketing/` folder. Product truth remains `docs/mvp/` (Bosnian homepage: top-nav homepage slot + existing hero + Pronađi salon + simple footer; discovery at `/salons`). Stack truth remains `docs/architecture/`. Code lives in `esyres_app/frontend/`.
